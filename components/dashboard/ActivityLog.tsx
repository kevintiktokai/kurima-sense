...
export interface Activity {
    ...
    recurring?: {
        ...
        dates?: string[]; // ISO dates for specific
    };
    fieldId?: string;
    aiSuggested?: boolean;
}
...
export const ActivityLog: React.FC<ActivityLogProps> = ({
    ...
    const [newActivity, setNewActivity] = useState({
        ...
        recurring: null as Activity['recurring'] | null,
        recurringType: 'none' as 'none' | 'daily' | 'weekly' | 'monthly' | 'specific',
    ...
    const handleComplete = async (id: string) => {
        ...
        try {
            ...
            // Revert on failure
            setActivities(prev => prev.map(a =>
                a.id === id ? { ...a, completed: false, completedAt: undefined } : a
            ));
        }
    };
        ...
        <div className="bg-white rounded-[2.5rem] p-6 lg:p-8 shadow-sm border border-slate-100">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-2xl p-3 mb-6">
                ...
                <div className="text-center">
                    ...
                    <p className="text-xs text-slate-400">{new Date(selectedDate).toLocaleDateString()}</p>
                </div>
                <button
                    onClick={() => navigateDate(1)}
                    className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center hover:border-brand-lime transition-colors"
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {completedToday.map(activity => (
                            <div
                                key={activity.id}
                                className="bg-emerald-50 border border-emerald-100 p-3 rounded-xl flex items-center gap-3 opacity-75"
                            >
                                <span className="w-5 h-5 bg-emerald-500 rounded-lg flex items-center justify-center text-white text-xs">✓</span>
                                <span className="text-sm line-through text-slate-500">{activity.title}</span>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-[2rem] p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="text-xs font-bold text-slate-400 uppercase block mb-2">Recurring</label>
                                <select
                                    value={newActivity.recurringType}
                                    onChange={e => setNewActivity(prev => ({ ...prev, recurringType: e.target.value as typeof newActivity.recurringType }))}
                                    className="w-full p-3 border border-slate-200 rounded-xl focus:border-brand-lime outline-none"
                                >
                            ...
                            {newActivity.recurringType === 'specific' && (
                                <div>
                                    <label className="text-xs font-bold text-slate-400 uppercase block mb-2">Select Dates</label>
                                    <input
                                        type="date"
                                        className="w-full p-3 border border-slate-200 rounded-xl focus:border-brand-lime outline-none"
                                        onChange={e => {
                                            if (e.target.value && !newActivity.specificDates.includes(e.target.value)) {
                                                setNewActivity(prev => ({
                                                    ...prev,
                                                    specificDates: [...prev.specificDates, e.target.value].sort()
                                                }));
                                            }
                                        }}
                                    />
                                    {newActivity.specificDates.length > 0 && (
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {newActivity.specificDates.map(date => (
                                                <span
                                                    key={date}
                                                    className="text-xs bg-slate-100 px-2 py-1 rounded-lg flex items-center gap-1"
                                                >
